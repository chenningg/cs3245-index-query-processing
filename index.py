...
def build_index(in_dir, out_dict, out_postings):
    ...
    dictionary = {}  # holding variable of format dictionary = {'term': {"docFreq": i, "termID": j, "postingListPointer": k}, ... }
        ...
        for sentence in sentences:
            ...
            for word in words_stemmed:
                if word not in dictionary: # new word/term, add to dictionary
                    dictionary[word] = {"docFreq": 1, "termID": termID, "postingListPointer": termID} # postingListPointer same as termID for now
                    postingLists.append([docID])
                    termID += 1
                else:  # word is in dictionary
                    if docID not in postingLists[dictionary[word]["postingListPointer"]]: # check if docID already in the corresponding postingList
                        postingLists[dictionary[word]["postingListPointer"]].append(docID)  # append docID to end of corresponding postingList if not exists
